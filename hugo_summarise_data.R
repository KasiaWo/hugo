...
hugo_summarise_data <- function(data,
  ...
  if(!getOption("hugo.know_summary_parameters" )){
    ...
    report_title <- title_input(data_name)
     ...
     print("wczytane domyslne parametry")
  ...
  if(overwrite_params){
    save_parametrs(default_parameters)
  }
...
get_parameters_from_user <- function(columns_name){
  ...
  if(!output %in% c("pdf", "html", "")){
    cat("Incorrect format of output. Default html.\n")
    output <- "html"
  }
  ...
  if(is.na(replace_output)){
    cat("Incorrect value. Default is TRUE.\n")
    replace_output <- TRUE
  }
  ...
  if(! (max_decimals %% 1 == 0) | is.na(max_decimals) ){
    cat("Incorrect value. Default is 2.\n")
    max_decimals <- TRUE
  }
